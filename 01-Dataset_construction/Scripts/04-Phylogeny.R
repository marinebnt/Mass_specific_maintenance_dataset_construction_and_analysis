...
pathoutput <- paste0( "01-Dataset_construction/Outputs/phylosem_output")
...
dataset_traits <-  read.csv(paste0(path, "/output_tot_stdmorpho/dataset_traits_phylosem.csv"), row.names = c(1))
...
# => This is done in advance, to gain simulation time
# phylogenetic tree
######
# INCOMPLETE PHYLOGENY
all <- fishtree::fishtree_complete_phylogeny(species=dataext$Species)
sp_no_phylo <- table(dataext$Order[which(stringr::str_replace(dataext$Species, " ","_") %in% tree$tip.label)])
fraction_per_order <- list()
for(sp in names(sp_no_phylo)){
  fraction_per_order[[sp]] <- sp_no_phylo[sp]/table(dataext$Order)[sp]
}
install.packages("rtrees")


write.tree(phy = all, file = paste0(path, "/Phylo_15216.tree"), append = FALSE, digits = 10, tree.names = FALSE) # format understood by the *ape* package
P = read.tree(paste0(path, "/Phylo_15216.tree"))
if (any(is.na(P))) {
...
# 
# par(mfrow=c(1,1))
# write.tree(phy = phylo_tree_med, file = paste0(path, "/Taxo_17003.tree"), append = FALSE, digits = 10, tree.names = FALSE) # format understood by the *ape* package
...
P <- read.tree(paste0(path, "/Taxo_17003.tree"))
P <- read.tree(paste0(path, "/Phylo_15216.tree"))
if(length(P) == 100){P=P[[1]]} #Phylogeny method produces 100 trees by stochastic assamblages : chose 1
