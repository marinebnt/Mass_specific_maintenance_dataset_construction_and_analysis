...
load(file=paste0(path_plots, "/ELASMO_PCA.RData"))
elasmo <- plot
load(file=paste0(path_plots, "/TELEO_PCA.RData"))
teleo <- plot
load(file=paste0(path_plots, "/TOT_PCA.RData"))
tot <- plot

plotall <- ggarrange(tot, teleo, NA, elasmo, labels = c("A","B","", "C"))

ggsave(file = paste0(path_plots, "/PCA_combined.pdf"), width=13, height=9.5)
