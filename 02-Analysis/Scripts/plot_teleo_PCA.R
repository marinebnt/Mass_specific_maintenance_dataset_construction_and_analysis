...
beta_iv1_tot = dataplot_noelasmo[, which(colnames(dataplot_noelasmo) %in%  traits)]
...
plot <- fviz_pca_biplot(res.pca,
  ...
  # scale_fill_manual(values = my_palette, name="MSM clusters")+
  # geom_text(data = datatoadd, aes(x = x + c(0.1,0.1,-1), y = y - c(0.4, 0.4,-0.5), label = z),                 
  ...
  guides(fill = "none", #guide_legend("Ellipsoid Fill"), 
         linetype = guide_legend("Class"),
         color = guide_legend(override.aes = list(fill = NA)))
plot


pdf(file = paste0(path_plots, "/PCA_teleo.pdf"), height = 8, width=6.8)
plot
dev.off()
...
fviz_pca_biplot(res.pca,
                        ...
                        habillage = , # color by groups
                        # palette = c("darkblue", "royalblue", "#00AFBB", "lightblue", "#E7B800", "#FC4E07", "darkred"),
                        col.ind = ifelse(dataacpPLOT$Genus == c("Gillichthys"), "red", "grey"), ##as.factor(dataacpPLOT[, c("colACP")]),
                        arrowsize = 0.25, 
...
save(plot, file=paste0(path_plots, "/TELEO_PCA.RData"))
...
# plot
mycol =  c("darkorchid4", "cyan3", "#4575b4", "#91bfdb", "#fee090", "#fc8d59","#d73027") #c("darkorchid4", "#4575b4", "#fee090", "#fc8d59") #c("darkorchid4", "plum3","#4575b4","#91bfdb", "#fee090", "#fc8d59", "#d73027") #"#91bfdb",fc8d59
plot <- fviz_pca_biplot(res.pca,
                        label = c("var"),# label = "none", # hide individual labels
                        habillage = , # color by groups
                        # palette = c("darkblue", "royalblue", "#00AFBB", "lightblue", "#E7B800", "#FC4E07", "darkred"),
                        #* col.ind = dataacpPLOT$c_m, #as.factor(round(dataacpPLOT$colACP, 2)),
                        #* gradient.cols = mycol,#col.ind = as.factor(round(dataacpPLOT$colACP, 2)),
                        col.ind = as.factor(round(dataacpPLOT$colACP, 2)), #*
                        arrowsize = 1.5, pointshape=19, labelsize = 5, alpha = 0.5,
                        col.var = "darkblue", repel= T)+
  scale_colour_manual(values = mycol, name="MSM cluster \ncentroid") + #*  # Adjust the color scale
