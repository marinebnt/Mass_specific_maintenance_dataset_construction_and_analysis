...
# colnames(AAAtot$archetypes) <- c("Age max", "K", "Infinite \nweight", "Trophic \nlevel", "Fecundity", "Habitat")
...
# colnames(AAAelasmo$archetypes) <- c("Age max", "K", "Infinite \nweight", "Trophic \nlevel", "Fecundity", "Habitat")
...
# TELEO
# AAAteleo <- AAAteleo2
# colnames(AAAteleo$archetypes) <- c("Age max", "K", "Infinite \nweight", "Trophic \nlevel", "Fecundity", "Habitat")
...
ggsave(plotteleo, file = paste0(path_plots, "/barplot_teleo.pdf"), width=5.63, height=4.03) #, width = 80, height = 180, dpi = 600, units = "mm"
...
mean_elas <- mean(dataphylo[which(dataphylo$Class == "Elasmobranchii"),"c_m"])
# ggplot() + 
#   geom_bar(aes(dataphylo$c_m, group=as.factor(dataphylo$Class)))
...
lm(dataphylo$c_m ~ dataphylo$Class)->a
#plot(a) # control diagnostic plots 
...
# BARPLOT AAA TOT
rownames(AAAtot$archetypes)<- c("Opportunistic", "Equilibrium", "Periodic")
# colnames(AAAtot$archetypes) <- c("Age max", "K", "Infinite \nweight", "Trophic \nlevel", "Fecundity", "Habitat")
archetype_traits_long <- as.data.frame(AAAtot$archetypes) %>%
  tibble::rownames_to_column(var = "Archetype") %>%
  pivot_longer(cols = -Archetype, names_to = "Trait", values_to = "Value")
...
# AAAteleo <- AAAteleo2
# colnames(AAAteleo$archetypes) <- c("Age max", "K", "Infinite \nweight", "Trophic \nlevel", "Fecundity", "Habitat")
rownames(AAAteleo$archetypes)<- c("Equilibrium", "Opportunistic", "Periodic")
archetype_traits_long <- as.data.frame(AAAteleo$archetypes) %>%
  tibble::rownames_to_column(var = "Archetype") %>%
  pivot_longer(cols = -Archetype, names_to = "Trait", values_to = "Value")
...
combined <- ggarrange(plottot, plotteleo, plot, ncol = 1, nrow=3, common.legend = T)
ggsave(combined, file=paste0(path_plots, "/barplot_combined.pdf"), width=7 , height=12)
